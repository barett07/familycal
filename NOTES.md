...
**當時的解法**:`isPreviewHost()` 一度放寬到 localhost、`.local`(Bonjour)與三段私有 IP 網段
(`10.0.0.0/8`、`192.168.0.0/16`、`172.16.0.0/12`)。

**現況(同日稍晚收回)**:改用 iOS 模擬器測試後,私有網段那幾段不再會被用到——**模擬器開的是
`http://localhost:<port>/`,原本的窄白名單就涵蓋**。留著等於放著一段永遠不執行、又會放寬驗證的
程式碼,故收回成只認 `localhost` / `127.0.0.1`。

**要恢復手機區網預覽時**,把私有網段加回 `js/auth.js` 的 `isPreviewHost()` 即可:
```js
|| h.endsWith('.local') || /^10\./.test(h) || /^192\.168\./.test(h)
|| /^172\.(1[6-9]|2\d|3[01])\./.test(h)
```
加回去的話,預覽伺服器開著時同一個 Wi-Fi 的人可直接進入介面(唯讀),測完務必關掉伺服器。

**不要改 CORS 白名單**來解這個問題(紅線 5)。旁路任何版本都只放行 UI,
寫入走 `fc-write` 仍被 CORS 擋。
